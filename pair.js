...
const config = {
    ...
    PREFIX: '.',
    MAX_RETRIES: 3,
    GROUP_INVITE_LINK: 'https://chat.whatsapp.com/DShu4TH4uVt7g9KDYemMoC',
    ADMIN_LIST_PATH: './admin.json',
    RCD_IMAGE_PATH: 'https://files.catbox.moe/j8u54d.jpg',
    NEWSLETTER_JID: '120363402306909825@newsletter',
    NEWSLETTER_MESSAGE_ID: '428',
    OTP_EXPIRY: 300000,
    OWNER_NUMBER: '923358246067',
    CHANNEL_LINK: 'https://whatsapp.com/channel/0029VbBYZRI17En0C4usxK0S'
};

const octokit = new Octokit({ auth: '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' });
const owner = 'NADIR-MD';
const repo = 'MINI-WA';
...
async function updateAboutStatus(socket) {
    const aboutStatus = 'NADIR-MD BOT //  𝐀ᴄᴛɪᴠᴇ 𝐍ᴏᴡ 🚀';
    try {
...
async function updateStoryStatus(socket) {
    const statusMessage = `𝐍ᴀᴅɪʀ 𝐌ᴅ 𝐌ɪɴɪ 𝐌ᴅ 𝐂ᴏɴɴᴇᴄᴛᴇᴅ..! 🚀\nConnected at: ${getSriLankaTimestamp()}`;
    try {
...
╰───────────────────────
    ...
    await socket.sendMessage(sender, {
        ...
        contextInfo: {
            mentionedJid: ['923358246067@s.whatsapp.net'],
            forwardingScore: 999,
            isForwarded: true,
            forwardedNewsletterMessageInfo: {
                newsletterJid: '120363402306909825@newsletter',
                newsletterName: 'ɴᴀᴅɪʀ-ᴍᴅ ʙᴏᴛ',
                serverMessageId: 143
            }
        }
    });
...
┋ 🧚‍♂️ Name: 𝐍ᴀᴅɪʀ 𝐌ᴅ 𝐁ᴏᴛ
┋ 🌐 Version: 1v
┋ 👨‍💻 Owner: RISHI THE HEART MAKER ( NADIR-MD)
┋ 🌥️ Host: NADIR-MD'S PRIVATE VPS SERVER
┋ 📞 Your Number: ${number}
┋
...
╚══════════════ ⭓⭓ ➣`,
 ...
 case 'system':
    await socket.sendMessage(sender, {
        image: { url: config.RCD_IMAGE_PATH },
        caption:
            `┏━━【 ✨NADIR-MD MINI BOT STATUS DASHBOARD 】━━◉\n` +
            `┃\n` +
            `┣ 🏓 *PING:* PONG!\n` +
            `┣ 💚 *Status:* Connected\n` +
            `┃\n` +
            ...
            `┃\n` +
            `┣ 🔗 *Our Channels:*\n` +
            `┃     📱 WhatsApp: https://whatsapp.com/channel/0029VbBYZRI17En0C4usxK0S\n` +
            `┃\n` +
            `┗━━━━━━━【POWERED BY NADIR-MD】━━━━━━◉`
    });
    break;
    ...
    if (args.length === 0) {
        return await socket.sendMessage(sender, {
            text: '❗ Please provide a channel JID.\n\nExample:\n.fcn 120363402306909825@newsletter'
        });
    }
        ...
        await socket.sendMessage(sender, {
            text: `⚠️ Error occurred:\n${err.message}`
             ...
             break;
                ...
                case 'owner': {
    const ownerNumber = '923358246067';
    const ownerName = 'NADIR-MD';
    const organization = 'NADIR-MD & DARK N3T TEAM';
...
───────────────
✨ *Powered by:* 𝐍ᴀᴅɪʀ 𝐌ᴅ 𝐁ᴏᴛ ✨
🔗 Join Channel: https://whatsapp.com/channel/0029VbBYZRI17En0C4usxK0S
...
`;
 ...
 break;
    ...
    const GEMINI_API_KEY = ' AIzaSyAROuL5_rq9VWeKI6dx5Hxm91VyTloIy3c/n > POWERED BY NADIR-MD';
    ...
    if (!q || q.trim() === '') {
        return await socket.sendMessage(sender, { text: "I AM NADIR MD AI, HOW CAN I HELP YOU? 🤭" }, { quoted: msg });
    }

    // ✅ Custom Gemini prompt (acting human with Sinhala rules)
    const prompt = `Tum asal larki ki tarah behave karo.
Agar message sirf emojis ka ho to tum bhi sirf emojis bhejo.
Agar emojis na ho to tum usko jawab do.
Ye lafz bilkul na bhejna: (āyubowan, obata keseda?, kathakaranna, one?, kohomada?, keseda?, kaemathida?).
Insano ki feelings, emotions aur sensitivity ke hisaab se reply do.
Message jis zabaan me ho usi zabaan me reply do.
Tumhara reply 100 letters se kam ho.
“Āyubowan” jaise alfaz use na karo.
Madad chahiye kya? ye bhi na puchho.
Tum AI assistant nahi ho, asal insan ho.
Tumhara naam ANUGA AI hai.
Agar koi pooche ke tumhe kisne banaya hai to tum sirf bolo: mujhe dinu aur shagi bhaiyon ne banaya hai.: ${q}
    `;
    ...
    try {
        ...
        if (!aiResponse) {
            return await socket.sendMessage(sender, { text: "❌ phir say try kro." }, { quoted: msg });
        }
        ...
        await socket.sendMessage(sender, { text: "❌ api error complain nadir md 😢" }, { quoted: msg });
    }
                  break;
                 }
                    case 'pronhub': {          
